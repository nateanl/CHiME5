...
chime5_corpus=/home/data/CHiME5
json_dir=${chime5_corpus}/transcriptions
audio_dir=${chime5_corpus}/audio

# training and test data
train_set=train_worn_u100k
train_beam_set=train_beam_ref
...
if [ $stage -le 7 ]; then
  ...
  mfccdir=mfcc
  for x in ${train_set} ${train_beam_set} ${test_sets}; do
...
if [ $stage -le 9 ]; then
  ...
  steps/train_mono.sh --nj $nj --cmd "$train_cmd" \
		      data/${train_beam_set} data/lang exp/mono_beam
fi

if [ $stage -le 10 ]; then
  steps/align_si.sh --nj $nj --cmd "$train_cmd" \
		    data/${train_beam_set} data/lang exp/mono_beam exp/mono_ali_beam

  steps/train_deltas.sh --cmd "$train_cmd" \
			2500 30000 data/${train_beam_set} data/lang exp/mono_ali_beam exp/tri1_beam
fi

if [ $stage -le 11 ]; then
  steps/align_si.sh --nj $nj --cmd "$train_cmd" \
		    data/${train_beam_set} data/lang exp/tri1_beam exp/tri1_ali_beam

  steps/train_lda_mllt.sh --cmd "$train_cmd" \
			  4000 50000 data/${train_beam_set} data/lang exp/tri1_ali_beam exp/tri2_beam
fi

if [ $stage -le 12 ]; then
  utils/mkgraph.sh data/lang exp/tri2_beam exp/tri2_beam/graph
  for dset in ${test_sets}; do
    steps/decode.sh --nj $decode_nj --cmd "$decode_cmd"  --num-threads 4 \
		    exp/tri2_beam/graph data/${dset} exp/tri2_beam/decode_${dset} &
  done
  wait
fi
...
if [ $stage -le 14 ]; then
  steps/align_si.sh --nj $nj --cmd "$train_cmd" \
		    data/${train_beam_set} data/lang exp/tri2_beam exp/tri2_beam_ali

  steps/train_sat.sh --cmd "$train_cmd" \
		     5000 100000 data/${train_beam_set} data/lang exp/tri2_beam exp/tri3_beam
fi

if [ $stage -le 15 ]; then
  utils/mkgraph.sh data/lang exp/tri3_beam exp/tri3_beam/graph
  for dset in ${test_sets}; do
    steps/decode_fmllr.sh --nj $decode_nj --cmd "$decode_cmd"  --num-threads 4 \
			  exp/tri3_beam/graph data/${dset} exp/tri3_beam/decode_${dset} &
  done
  wait
fi
...
if [ $stage -le 16 ]; then
  ...
  steps/cleanup/clean_and_segment_data.sh --nj ${nj} --cmd "$train_cmd" \
    ...
    data/${train_set} data/lang exp/tri3_beam exp/tri3_cleaned_beam data/${train_set}_cleaned
fi

if [ $stage -le 17 ]; then
  # chain TDNN
  local/chain/run_tdnn.sh --nj ${nj} --train-set ${train_set}_cleaned --test-sets "$test_sets" --gmm tri3_cleaned_beam --nnet3-affix _${train_set}_cleaned
fi
